...
<center>
<img src="/uploads/Sweep-Line_img1.png" width="300px">
</center>
...
=== "Python"
    ...
    squared_d = squared_dist(points[0], points[1])
    res_id1, res_id2 = 0, 1

    points.sort()

    # Use a sorted list of (y, x, id) for the sweep line
    T = []

    for p in points:
        x, y, pid = p
        d = int(math.sqrt(squared_d))
        
        # Remove points with x-coordinate difference > d
        T = [(ty, tx, tid) for ty, tx, tid in T if tx >= x - d]
        
        # Find points with y in [y - d, y + d]
        lo = bisect.bisect_left(T, (y - d, -10**18, -1))
        hi = bisect.bisect_right(T, (y + d, 10**18, n))
        
        for i in range(lo, hi):
            ty, tx, tid = T[i]
            cur = (tx, ty, tid)
            sd = squared_dist(p, cur)
            if sd < squared_d:
                squared_d = sd
                res_id1 = pid
                res_id2 = tid
        
        bisect.insort(T, (y, x, pid))

    if res_id1 > res_id2:
        res_id1, res_id2 = res_id2, res_id1
    print(res_id1, res_id2, f"{math.sqrt(squared_d):.6f}")
    ```
...
<center>
<img src="/uploads/Sweep-Line_img3.png" width="200px">
</center>
...
=== "Python"
    ...
    for i in range(1, n + 1):
        ...
        if x1 > x2:
            x1, x2 = x2, x1
        blue_x1[i], blue_x2[i], blue_y[i] = x1, x2, y
        compress_y.append(y)

    for i in range(1, n + 1):
        y1, y2, x = map(float, input().split())
        if y1 > y2:
            y1, y2 = y2, y1
        red_y1[i], red_y2[i], red_x[i] = y1, y2, x
        compress_y.append(y1)
        compress_y.append(y2)

    compress_y = sorted(set(compress_y))

    for i in range(1, n + 1):
        yy = bisect.bisect_left(compress_y, blue_y[i]) + 1
        events.append((blue_x1[i] - EPS, yy, 0, 1))
        events.append((blue_x2[i] + EPS, yy, 0, 2))

    for i in range(1, n + 1):
        l = bisect.bisect_left(compress_y, red_y1[i]) + 1
        r = bisect.bisect_left(compress_y, red_y2[i]) + 1
        events.append((red_x[i], l, r, 3))
...
<center>
<img src="/uploads/Sweep-Line_img4.png" width="300px">
</center>
...
=== "Python"
    ...
    class SegmentTree:
        def __init__(self, n):
            self.s = [[0, 0] for _ in range(n * 4 + 5)]

        def update(self, id, l, r, tl, tr, val):
            if l > tr or r < tl:
                return
            if l >= tl and r <= tr:
                self.s[id][1] += val
                if self.s[id][1] != 0:
                    ...
                    self.s[id][0] = self.s[id * 2][0] + self.s[id * 2 + 1][0]
                else:
                    self.s[id][0] = 0
                return
            m = (l + r) // 2
            self.update(id * 2, l, m, tl, tr, val)
            self.update(id * 2 + 1, m + 1, r, tl, tr, val)
            if self.s[id][1] != 0:
                self.s[id][0] = r - l + 1
            else:
                self.s[id][0] = self.s[id * 2][0] + self.s[id * 2 + 1][0]
    ...
    for i in range(len(segments) - 1):
        x, y1, y2, typ = segments[i]
        ST.update(1, 0, MX, y1, y2 - 1, typ)
        res += (segments[i + 1][0] - segments[i][0]) * ST.s[1][0]
...
def solve_single_recur(p):
    if len(p) <= 1:
        return p
    upper_size = len(p) // 2
    upper = solve_single_recur(p[:upper_size])
    lower = solve_single_recur(p[upper_size:])
    res = []
    min_diff_yx = (0, INF, -1)
    upper_ptr = 0
    for lo in lower:
        while upper_ptr < upper_size and upper[upper_ptr][0] + upper[upper_ptr][1] <= lo[0] + lo[1]:
            if min_diff_yx[1] - min_diff_yx[0] > upper[upper_ptr][1] - upper[upper_ptr][0]:
                min_diff_yx = upper[upper_ptr]
            res.append(upper[upper_ptr])
            upper_ptr += 1
        if min_diff_yx[2] != -1:
            potential_edges.append((min_diff_yx[2], lo[2], manhattan_dist(min_diff_yx, lo)))
        res.append(lo)
    res.extend(upper[upper_ptr:])
    return res

def solve_single(p):
    p.sort(key=lambda pt: (-pt[1], pt[0]))
    solve_single_recur(p)
...
def solve(p):
    for ft in range(2):
        for _ in range(4):
            ...
            rotate_90(p)
        flip(p)
    potential_edges.sort(key=lambda e: e[2])
    dsu = DSU(len(p))
    ans = 0
