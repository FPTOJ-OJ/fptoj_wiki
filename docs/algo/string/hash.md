...
Một lớp những bài toán rất được quan tâm trong khoa học máy tính nói chung và lập trình thi cử nói riêng, đó là [xử lý xâu chuỗi](basic.md). Trong lớp bài toán này, người ta thường rất hay phải đối mặt với một bài toán: tìm kiếm xâu chuỗi.
...
-  **[Knuth-Morris-Pratt](../../translate/wcipeg/kmp.md)**: Hay còn được viết tắt là KMP, được phát minh vào năm 1974, bởi Donald Knuth, Vaughan Pratt và James H. Morris.  Thuật toán này sử dụng một correction-array, là một thuật toán rất hiệu quả, có độ phức tạp là $O(m + n)$.
...
1. Ở đây mình đổi chữ `a` thành số 1 chứ không phải số 0. Đây là chi tiết vô cùng quan trọng, để tránh 2 xâu: `abc` và `bc` bằng nhau khi đổi ra số. Bạn có thể đọc thêm chi tiết ở phần [Chi tiết cài đặt](#chi-tiet-cai-at).
2. Thông thường ta chọn $base$ là một số nguyên tố. Mình sẽ giải thích thêm trong phần [Chi tiết cài đặt](#chi-tiet-cai-at).
...
**Lưu ý**: Lý do chọn $MOD$ là số nguyên tố được giải thích thêm trong phần [Chi tiết cài đặt](#chi-tiet-cai-at).

Dễ dàng nhận thấy việc so sánh $x \bmod MOD$ với $y \bmod MOD$ rồi kết luận $a$ có bằng với $b$ hay không là sai. $x \bmod MOD = y \bmod MOD$ chỉ là điều kiện cần để $a$ bằng $b$ chứ chưa phải điều  kiện đủ. Tuy nhiên, chúng ta sẽ chấp nhận lập luận sai này trong thuật toán Hash. Và coi điều kiện cần như điều kiện đủ. Trên thực tế, lập luận sai này có thể dẫn đến kết quả sai nếu bạn không hiểu rõ mình đang làm gì. Để hiểu rõ về tỉ lệ sai của thuật toán Hash, các bạn đọc thêm phần [Đánh giá độ chính xác](#anh-gia-o-chinh-xac). Phần [Chi tiết cài đặt](#chi-tiet-cai-at) cũng nói thêm về cách tránh bị sai số khi cài đặt Hash.
...
Nếu bạn không hiểu rõ về cách đánh giá độ chính xác của thuật Hash (trình bày ở mục [Đánh giá độ chính xác](#anh-gia-o-chinh-xac)), bạn chỉ cần chọn 3-4 số nguyên tố khác nhau làm $MOD$. Bạn có thể tham khảo [code của Phạm Văn Hạnh](http://codeforces.com/contest/727/submission/21451704). Tuy nhiên các bạn cũng nên lưu ý là dùng nhiều $MOD$ quá cũng làm chương trình chạy chậm đi.
...
- Chọn $MOD$ đủ lớn theo nguyên tắc đã phân tích ở phần [Đánh giá độ chính xác](#anh-gia-o-chinh-xac).
