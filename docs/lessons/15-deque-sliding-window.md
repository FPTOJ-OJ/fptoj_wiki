...
![Sliding Window](../uploads/img/sliding-window.svg)
*Minh họa kỹ thuật Sliding Window*
...
i=0: a[0]=1, deque=[0]           → cửa sổ chưa đầy
i=1: a[1]=3, loại a[0]=1 ≤ 3 → deque=[1]       → cửa sổ chưa đầy
i=2: a[2]=-1, deque=[1,2]        → max=a[1]=3 ✅
i=3: a[3]=-3, deque=[1,2,3]      → i=1 thoái → deque=[2,3] → max=a[2]=-1? 
     → Sai! deque giảm dần: a[2]=-1 > a[3]=-3 → deque=[2,3] → max=a[1]=3? 
     → Kiểm tra: front=2, a[2]=-1, nhưng deque=[1,2,3] rồi front=1 bị loại vì 1<=3-3=0
     → deque=[2,3], front=2, a[2]=-1... 
     → Thực tế: i=3, front nên = max trong [3,-1,-3] = 3 → deque phải có chỉ số 1
     → Nhưng chỉ số 1 đã bị loại vì 1 <= 3-3 = 0 → CỬA SỔ [3,-1,-3]
     → deque: i=1 bị loại (1<=0), i=2: a[2]=-1, i=3: a[3]=-3 → deque=[2,3]
     → max = a[2] = -1? Sai! 
     
     Chạy lại đúng:
     i=0: dq=[0], a[0]=1
     i=1: loai dq.back=0 vi a[0]=1<=a[1]=3 → dq=[1]
     i=2: a[2]=-1, dq=[1,2], i>=2 → result: a[1]=3
     i=3: front=1, 1<=3-3=0? 1<=0 → SAI, 1>0 → khong loai front
          a[dq.back=2]=-1 > a[3]=-3 → khong loai → dq=[1,2,3]
          i>=2 → result: a[1]=3
     i=4: front=1, 1<=4-3=1 → loai → dq=[2,3]
          a[3]=-3 <= a[4]=5 → loai 3 → dq=[2]
          a[2]=-1 <= a[4]=5 → loai 2 → dq=[4]
          result: a[4]=5
     i=5: dq=[4,5], result: a[4]=5
     i=6: a[5]=3<=6 → loai 5; a[4]=5<=6? 5<=6 → loai 4; dq=[6] → result: a[6]=6
          SAI! a[4]=5<6 → loai; dq=[6], result: 6
          Nhung [5,3,6] max=6 ✅
     i=7: dq=[6,7], result: a[6]=6? → [3,6,7] max=7
          a[7]=7, a[6]=6<=7 → loai 6 → dq=[7] → result: a[7]=7 ✅
