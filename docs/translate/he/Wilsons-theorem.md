...
$$
a \cdot a^{n-2} = a^{n-1} \equiv 1 \pmod{n}
$$
Đặt $b = a^{n-2} \bmod n$. Với mỗi $a$ thì $b$ là duy nhất và $b < n$ để $a \cdot b \pmod{n} = 1$, mặt khác $a=b$ khi và chỉ khi $a=1$ hoặc $a=n-1$ nên ta có thể tạo ra $\frac{n-2}{2}$ cặp số $a, b$ phân biệt như vậy. Nhân tất cả các cặp với nhau ta được

$2 \cdot 3 \cdot 4 \cdot \ldots \cdot (n-2) \bmod n = 1$ 

$\Rightarrow 1 \cdot 2 \cdot 3 \cdot \ldots \cdot (n-1) \bmod n = n-1$
